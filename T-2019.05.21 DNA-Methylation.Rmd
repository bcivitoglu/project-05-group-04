...
### Rename all the (old) columns at once ("Chromosome" --> "Chr" ; "CpG" --> "CpG´s")

names(dfnew1) <- c("Chr", "CpG ´s")
...
###1 Set all Zero to NA´s (due to possible mistakes)

df[df == 0] <- NA

###2 Go through each row and sum up the NA´s
...
###4 Remove your old Dataset with the NA´s with your new Dataset without the NA´s (Remove any row with more than i NA´s)
...
--> Frage: Wieso habe ich in df_new bei jeder Spalte nur die 95%
  Quantile, obwohl ich ein df$specialcolumn ausgewählt habe ?
