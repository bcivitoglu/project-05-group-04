...
P-values under 0.1 are significant. To make visualisation easier we set values above the significance threshold to 10 and values equal or below to 1, so we just have to find colours for 2 values.
...
Therefor we go on with our analysis using Principal component 2 which doesn't show a batch effect at all.
...
showing the most important 10000 genes in PC2 in ranked order with loading scores with positive and negative sign
```{r}
...
We create a dataframe containing all M-values from the 10000 most important genes for principal component 2.
...
By performing k-means clustering we want to see whether sick and healthy patients are separated in two different clusters????
```{r}
