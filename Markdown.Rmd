...
The p-values from the t-test were corrected with the holm method. 24 genes had a p-value under 0.1. Because of that we defined this genes of healthy patients as significantly differently methylated than the ones of sick patients.
...
We have had created a dataframe `relevant_genes` containing the genes which show significant differences in methylation and four genes, which do not show significant methylation differences between sick and healthy, but seem to be worth a look, because we read about them in literature before. But to know wether those genes in cancer patients are hyper or hypomethylated we need to have a look at the fold value. Therefor we **add the fold values to the relevant genes**.
Genes in cancer patients are **upmethylated** if the **fold is negative** and the absolute fold higher than 1.
And Genes are **downmethylated** if the **fold is positiv** and the absolute fold higher than 1.
