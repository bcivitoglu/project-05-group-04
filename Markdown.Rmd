...
5. **Identifying DMRs**
  *t-test and p-value correction
  *testing for hypo- and hypermethylation
...
```
### We check if important genes are in our Dataset which we found in literature
```{r}
...
The Wilcoxon test for dependent spot checks tests, whether the central tendencies are different. "Dependent spot checks" is used when one measurement in a sample and one measurement in another sample influence each other. In three situations this is the case:
1) Repeated measures
2) Natural pairs
3) Matching
...
Explanation of Kruskal-Wallis test
...
For analyzing and comparing all p-values we state all p-values under 0.1 as significant. To make visualisation easier we set values above the significance threshold to 10 and values equal or below to 1, so we just have to find colours for 2 values.
...
```{r}
View(pca_M$rotation[genes_top_10000,2]) muss eventuell raus mein R spinnt nur spuckt wahrscheinlich lange sachen aus
```
...
Actually there is a knik in the elbow plot of loading scores of the genes at round about 2000 genes. K-means clustering already worked with 2000 genes but by using only 2000 genes we get only 3 significant different methylated genes, which is not sufficient. By using 10000 genes we get 31 different methylated genes, which is an amount with which we can work with. 

#####create a dataframe containing all M-values from the 10000 most important genes for principal component 2.
...
K-means is a method to find groups in the data. It uses an iterative Algorithm which means that there is a repitition of steps with the aim of finding the right cluster for each datapoint. First step is the clustering which means that the datapoints get split up into k clusters. Each datapoint gets allot the its next clusterpoint. Next step is an updating of the clusters by calculating center of each cluster and define them as new clusterpoints. These two steps will be repeated as long as one set in the code. By performing k-means clustering we want to **ckeck whether sick and healthy patients are separated in two different clusters.**
...
To show them in rising order, this code can be used. This is interesting to get an impression, how many genes are significantly varying in methylation, but doing this changes the order of genes, so the information about which gene it is gets lost. Therefor we won't go on with this increasing order
...
To reduce the family-wise error rate, which is the propability of getting one ore more type 1 errors (also called false positive error), the *holm method* is used. Besides a *dataframe* containing the uncorrected p-values, the corrected p-values and the belonging gene ID is created.
...
Only 31 genes are significantly differently methylated,
####Statistical and biological relevant methylation differences in genes

A vulcano plot can help visualising whether a gene is claimed to be differently methylated in healthy and sick patients because of statistical reasons, which means a significant p-value or because of biological reasons, which would be a big difference in the mean of the M-values. Only the genes which show both characteristics, really have a significant methylation difference.

#####Fold change calculation

On the x-axis of the volcano plot there is the **difference of the M-Value means of each sample group**. This is the axis showing the biological relevance. Usually for better visualization you use the log2 fold change. Due to the negative M-values that exist, this is mathematically not possible, which you see running the first code. Therefor we dont use the log2 fold change, but only the mean-difference.
...
To visualise genes that are statistically and biologically relevant and subdivide them into upmethylated and hypomethylated genes in case of cancer, we first **create thresholds for significance** in p-values and M-value-difference (fold).
...
We create a **filter for biological relevant genes** using the threshold for M-value-difference, one for **statistically relevant genes** and one that **combines both in one filter**.
