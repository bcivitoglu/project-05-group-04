...
To **find an upper threshold** for deleting coverage values we try quantile values used in literature and draw the lines for those quantiles in the logarithmic plot to see whether one of those coincidences with a kink in the curve. For the **lower threshold** we draw lines at the values 10 and 15, which are common threshold values as we read in literature.
...
The **literature genes still existing in the dataframe after the t-test** are those four ones. They don't show a significant p-value, but still we will do some research on them.
