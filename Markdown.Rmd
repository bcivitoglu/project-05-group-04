...
Batch effect is a **technical source of error**. It is important that the technical variation does not confound with the biology to not falsify the interpreted data.

At first we investigated for a possible batch effect by looking at the categories gender, sample desc, biomaterial provider and age. As age is not a perfect example of batch effect, [...] it is not further investigated.
You may be able to guess the batch effect by taking a look at the degree of separation. If all the data points of one variable are close to each other and they are not well separated by the principal component, this could indicate a batch effect in the respective components.  
...
We give to the columns and rows informative names.
...
For analyzing and comparing all p-values we choose as threshold for significancy 0.1 because this is a common threshold applied in statistical tests. To make visualization easier we set values above the significance threshold to 10 and values equal or below to 1, so we just have to find colours for 2 values.
...
By looking at the heatmap or at the p-values one can see that principal component 1 has batch effect: sample desc and biomaterial provider. 
...
K-means is a method to find groups in the data. It uses an iterative algorithm which means that there is a repetition of steps with the aim of finding the right cluster for each datapoint. In the first step the datapoints are split into k clusters (k chosen by the user). Each datapoint is assigned to one of the k clusters. Next step is an updating of the clusters by calculating center of each cluster and define them as new clusterpoints. These two steps will be repeated as long as no datapoint change the belonging cluster. By performing k-means clustering we want to **check whether sick and healthy patients are separated in two different clusters.** Therefore we first create two dataframes containing all M-values from the 2000 and 10000 most important genes for principal component 2.
...
#### Significant different methylated genes

The p-values from the t-test were corrected with the holm method. 24 genes had a p-value under 0.1 (threshold chose for significancy). Because of that, we defined this genes of as significantly differently methylated between healtht and sick patients.
The most differently methylated gene is MIR-941-1. It is a gene for a microRNA (non-coding and short RNA) that plays a role in the transcriptional regulation of gene expression. MicroRNAs have an influence on stability and translation of mRNAs by being a part of the RNA induced silencing complex (RISC). RISC can bind mRNAs complementary to its miRNA or siRNA. This can either inhibit or destabilize the mRNA, so that less or no proteins coded by the mRNA get produced. MIR-941-1 is known to be transcriptionally silenced in gastric cancer cells which fits to our results. Other differently methylated genes in our dataset which are coding for miRNAs were MIR3687-1, MIR1915, MIR8075, MIR941-2, MIR941-3, MIR941-4 and MIR647. 
...
#### Genes we found in literature which are not significant different nethylated
...
#### Gene number one: ACTBP12

ACTBP12 is just a pseudogene with no specific function. As the gene out of our literature analysis was named âCTBP2â and not âACTBP12â, this is not surprising. Still we wanted to see, whether there is a relation between these genes.
...
We have had created a dataframe `relevant_genes` containing the genes which show significant differences in methylation and four genes, which do not show significant methylation differences between sick and healthy, but seem to be worth a look, because we read about them in literature before. But to know whether those genes in cancer patients are hyper or hypomethylated we need to have a look at the fold value. Therefore we **add the fold values to the relevant genes**.
